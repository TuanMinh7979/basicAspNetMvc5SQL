(function (root) {
	...
	Pager.prototype.click = function (e) {
		...
		if (item) {
			...
			///
			let newPageNum = item.getAttribute("data-page")
			// changePage(6, newPageNum);
			///
			if (item.ellipsis) {
				return;
			}
...
const config = {
	...
	ellipsisText: '<i class="fa fa-ellipsis-h" aria-hidden="true"></i>',
};

const pager = new Pager(config)
pager.render(parseInt(nofPage));
